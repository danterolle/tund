                       ...
                       const peerforge_session_t *session)
{
    ...
    int sent = (int)sendto(sock, (const char *)buf, (size_t)len, 0,
                           (const struct sockaddr *)&session->server,
                           sizeof(session->server));
    return sent == len ? 0 : -1;
}

static int recv_message(peerforge_socket_t sock, const peerforge_session_t *session,
                        ...
                        uint8_t *type, uint16_t *payload_len)
{
    ...
    int len = (int)recvfrom(sock, (char *)buf, TUND_MAX_PKT, 0,
                            (struct sockaddr *)&from, &from_len);
    if (len < TUND_HDR_SIZE)
        return -1;
    if (!proto_verify(buf, len, session->key0, session->key1))
        return -1;
    if (proto_read_hdr(buf, type, payload_len) < 0)
        return -1;
    if (TUND_HDR_SIZE + *payload_len > len)
        return -1;
    return len;
}

static void handle_server_keepalive(peerforge_socket_t sock,
                                    const peerforge_session_t *session,
                                    ...
                                    uint16_t payload_len)
{
    ...
    if (!proto_read_keepalive_timestamp(payload, payload_len, &timestamp))
        return;
    int len = proto_build_keepalive_ack(buf, timestamp);
    send_signed(sock, buf, len, session);
}

void peerforge_client_drain(peerforge_client_t *client,
                            const peerforge_session_t *session)
{
    for (;;) {
        uint8_t buf[TUND_MAX_PKT];
                              ...
                              const peerforge_session_t *session)
{
    ...
    if (send_signed(client->sock, buf, len, session) < 0)
        return -1;
    ...
    while (peerforge_now_ms() < deadline) {
        ...
        len = recv_message(client->sock, session, buf, remaining > 0 ? remaining : 1,
                           &type, &payload_len);
        if (len <= 0)
            continue;
        if (type == MSG_KEEPALIVE) {
                              ...
                              const peerforge_session_t *session)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (send_signed(clients[i].sock, buf, len, session) < 0)
            continue;
        ...
        while (peerforge_now_ms() < deadline) {
            ...
            uint16_t payload_len = 0;
            int remaining = (int)(deadline - peerforge_now_ms());
            len = recv_message(clients[i].sock, session, buf,
                               remaining > 0 ? remaining : 1,
                               &type, &payload_len);
            if (len <= 0)
                continue;
            if (type == MSG_KEEPALIVE) {
                handle_server_keepalive(clients[i].sock, session,
                                        buf + TUND_HDR_SIZE, payload_len);
            } else if (type == MSG_KEEPALIVE_ACK) {
                uint64_t echoed = 0;
                if (proto_read_keepalive_timestamp(buf + TUND_HDR_SIZE,
                                                   payload_len, &echoed) &&
                    echoed == timestamp) {
                    ok++;
                    break;
                         ...
                         const peerforge_session_t *session)
{
    ...
    for (int i = 0; i < pairs; i++) {
        ...
        if (send_signed(clients[src_idx].sock, buf, len, session) < 0)
            continue;
        ...
        while (peerforge_now_ms() < deadline) {
            ...
            uint16_t payload_len = 0;
            int remaining = (int)(deadline - peerforge_now_ms());
            len = recv_message(clients[dst_idx].sock, session, buf,
                               remaining > 0 ? remaining : 1,
                               &type, &payload_len);
            if (len <= 0)
                continue;
            if (type == MSG_KEEPALIVE) {
                handle_server_keepalive(clients[dst_idx].sock, session,
                                        buf + TUND_HDR_SIZE, payload_len);
            } else if (type == MSG_DATA && payload_len >= sizeof(ip_pkt) &&
                       proto_get_src_ip(buf + TUND_HDR_SIZE, payload_len) == clients[src_idx].virt_ip &&
                       proto_get_dst_ip(buf + TUND_HDR_SIZE, payload_len) == clients[dst_idx].virt_ip) {
                ok++;
                break;
            }
        }
