...
static void close_clients(peerforge_client_t *clients, int count)
{
    for (int i = 0; i < count; i++)
        peerforge_client_close(&clients[i]);
}

static int run_peerforge(const peerforge_options_t *opts)
{
    peerforge_session_t session;
    if (peerforge_resolve_server(opts->server, opts->port, &session.server) < 0)
        return 1;
    ...
    for (int i = 0; i < opts->clients; i++)
        peerforge_client_init(&clients[i]);
    ...
    int data_ok = peerforge_data_probe(clients, opts->clients,
                                       opts->data_pairs, &session);
    ...
    if (opts->keepalives > 0)
        printf("keepalive acks: %d/%d\n", keepalive_ok,
               registered * opts->keepalives);
    if (opts->data_pairs > 0)
        printf("data probes: %d/%d\n", data_ok, opts->data_pairs);

    close_clients(clients, opts->clients);
    free(clients);

    if (registered != opts->clients)
        return 1;
    if (opts->keepalives > 0 && keepalive_ok != registered * opts->keepalives)
        return 1;
    if (opts->data_pairs > 0 && data_ok != opts->data_pairs)
        return 1;
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    if (!peerforge_parse_options(argc, argv, &opts))
        return 2;

    if (peerforge_net_init() < 0)
        return 1;
