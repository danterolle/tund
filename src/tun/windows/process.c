...
int run_cmd(const char *exe, const char *args)
{
    ...
    if (wait == WAIT_FAILED) {
        DWORD err = GetLastError();
        LOG_ERROR("Failed while waiting for command '%s': %s (error %lu)", full, win32_errstr(err), err);
    ...
    if (!GetExitCodeProcess(pi.hProcess, &code)) {
        DWORD err = GetLastError();
        LOG_ERROR("Could not read exit code for command '%s': %s (error %lu)", full, win32_errstr(err), err);
        CloseHandle(pi.hProcess);
        CloseHandle(pi.hThread);
        return -1;
    }
