...
static inline int sock_close(socket_t s) {
    return closesocket(s);
}
static inline int sock_valid(socket_t s) {
...
static inline const char *sock_errstr(int err) {
    ...
    if (buf[0] == '\0') snprintf(buf, sizeof(buf), "WSA error %d", err);
    size_t n = strlen(buf);
    while (n > 0 && (buf[n - 1] == '\n' || buf[n - 1] == '\r' || buf[n - 1] == '.'))
        buf[--n] = '\0';
    return buf;
...
static inline const char *ip_to_str_buf(uint32_t ip_nbo, char *buf, size_t len) {
    struct in_addr a;
    a.s_addr = ip_nbo;
    if (!inet_ntop(AF_INET, &a, buf, (socklen_t)len)) snprintf(buf, len, "<invalid>");
    return buf;
}

static inline uint64_t smooth_rtt_ms(uint64_t current, uint64_t sample, bool has_current) {
    if (!has_current) return sample;
    return (current * TUND_RTT_SMOOTHING_WEIGHT + sample + TUND_RTT_SMOOTHING_WEIGHT / 2) /
