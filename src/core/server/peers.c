...
void server_snapshot_peer_locked(const peer_t *peer, int idx,
...
int server_snapshot_broadcast_locked(const server_t *srv,
                                     server_peer_snapshot_t *out,
                                       ...
                                       server_peer_snapshot_t *out)
{
    ...
    if (idx >= 0)
        server_snapshot_peer_locked(&srv->peers[idx], idx, out);
    return idx;
}

void server_add_peer_bytes_out(server_t *srv,
                               const server_peer_snapshot_t *peer,
                               uint64_t bytes_out)
{
    if (bytes_out == 0)
        return;
...
void server_send_peer_snapshots(server_t *srv,
                                const server_peer_snapshot_t *peers,
                                int peer_count, uint8_t *buf, int len,
                                uint64_t bytes_out)
{
    for (int i = 0; i < peer_count; i++) {
        if (net_send(srv->sockfd, buf, len, &peers[i].real_addr) < 0) {
            char peer_ip[TUND_IP_STR_LEN];
            LOG_WARN("Send to %s (%s) failed",
                     peers[i].name,
...
void server_send_peer_list(server_t *srv, int peer_idx)
{
    ...
    pthread_mutex_lock(&srv->peers_lock);
    if (peer_idx >= 0 && peer_idx < TUND_MAX_PEERS &&
        srv->peers[peer_idx].active) {
    ...
    for (int i = 0; i < count; i++)
        memcpy(payload + i * (int)sizeof(msg_peer_entry_t),
