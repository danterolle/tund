...
int net_send(socket_t sockfd, uint8_t *buf, int len, const struct sockaddr_in *dest) {
    ...
    if (tund_test_send_count < TUND_TEST_MAX_SENDS) {
        ...
        if (len > 0 && len <= (int)sizeof(record->buf)) memcpy(record->buf, buf, (size_t)len);
        if (len >= TUND_HDR_SIZE) proto_read_hdr(buf, &record->type, &record->payload_len);
    }
...
int tun_write(tun_device_t *dev, const uint8_t *buf, int len) {
    ...
    if (tund_test_tun_write_count < TUND_TEST_MAX_TUN_WRITES) {
        tund_test_tun_write_record_t *record = &tund_test_tun_writes[tund_test_tun_write_count];
        record->len = len;
        if (len > 0 && len <= (int)sizeof(record->buf)) memcpy(record->buf, buf, (size_t)len);
    }
...
void tund_test_set_server_peer(server_t *srv, int idx, uint32_t vip_host, uint32_t real_host,
    ...
    if (name) snprintf(srv->peers[idx].name, TUND_NAME_LEN, "%s", name);
    proto_replay_reset(&srv->peers[idx].replay);
    if (!was_active) srv->peer_count++;
}
